...
def say_welcome(message):
    ...
    bot.send_message(message.chat.id,"""\n
/promt1 - промт 1 в запросе будит учитоваться этот промт\n
/promt2 - промт 2 в запросе будит учитоваться этот промт\n
/context сбросит контекст по текущей модели\n
/addpromt добавление нового промта""",
                     parse_mode='markdown')
...
def send_button(message):
    promt = sql.select_query('user', f"id={message.chat.id}")[0]['promt'].decode('utf-8')
...
def work_promt1(message):
    #payload = sql.get_payload(message.chat.id)
    promt = sql.select_query('promt', f'promt="promt1"')[0]['url'].decode('utf-8') 
    ...
    bot.send_message(message.chat.id,f'вы работаете по промту {promt}',reply_markup=create_menu_keyboard())

@bot.message_handler(commands=['promt2'])
def work_promt2(message):
    #payload = sql.get_payload(message.chat.id)
    promt = sql.select_query('promt', f'promt="promt2"')[0]['url'].decode('utf-8') 
...
def any_message(message):
    ...
    #try:
    promt = sql.select_query('user', f'id={message.chat.id}')[0]['promt'].decode('utf-8')
    promtUrl = sql.select_query('promt', f'promt="{promt}"')[0]['url'].decode('utf-8')
